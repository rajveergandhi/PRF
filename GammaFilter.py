...
class GammaFilter():
    ...
    def __init__(self):
        self.name = "Gamma Filter"
        self.description = "A filter which provides a smoothing effect and with which the visual appearance of the speckle is reduced."
        ...
        for band in xrange(self.bandCount):
            for i in xrange(row):
                ...
                for j in xrange(col):
                    ...
                    new_pix_value = np.real(np.roots([1,-window_mean,window_std*window_std,(window_std * pix_value * window_std)]))
                    outBlock[band][i, j] = np.round(new_pix_value[0])
    ...
    def updateKeyMetadata(self, names, bandIndex, **keyMetadata):
        if bandIndex == -1:                             # dataset-level properties
            ...
            keyMetadata['bandname'] = 'GammaFilter'
        return keyMetadata
...
"""
References:
    https://www.researchgate.net/publication/239959635_Effects_and_performance_of_speckle_noise_reduction_filters_on_active_radar_and_sar_images
"""