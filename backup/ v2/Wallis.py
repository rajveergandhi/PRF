...
class Wallis():
    ...
    def updatePixels(self, tlc, shape, props, **pixelBlocks):
        ...
        for band in xrange(self.bandCount):
            ...
            # apply wallis normalization filter
            #outBlockBand = (self.alpha * self.dsMean) + ((1 - self.alpha) * localmean) + ((rBand - localmean) * self.dsStd/((1 / self.maxGain) + localstddev))
            outBlockBand = np.sqrt(self.dsStd/((1 / self.maxGain) + localstddev)) * (rBand - localmean) + (self.alpha * self.dsMean) + ((1 - self.alpha) * localmean) 
            outBlockBand = outBlockBand.reshape((row,col))

            # output pixel array with wallis filter applied
            outBlock[band] = outBlockBand
